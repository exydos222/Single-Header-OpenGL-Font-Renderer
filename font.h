...
const inline void InitFontRenderer(unsigned short windowWidth, unsigned short windowHeight, const char* fontName, unsigned short minASCII = 32, unsigned short maxASCII = 126) {
    ...
    for (unsigned short c = minASCII; c < maxASCII; c++)
    {
        ...
        Characters.insert(std::pair<char, Character>(c, Character{ vec2f{ (float)face->glyph->bitmap.width / windowWidth, (float)face->glyph->bitmap.rows / windowHeight }, vec2f{ (float)face->glyph->bitmap_left / windowWidth, (float)face->glyph->bitmap_top / windowHeight }, vec2f{ (float)ox / w, (float)oy / h }, vec2f{ (float)face->glyph->bitmap.width / w, (float)face->glyph->bitmap.rows / h }, (float)face->glyph->advance.x / 64 / windowWidth }));
